---
layout: default-layout
title: Main Page - Dynamsoft Code Parser Module .NET Edition API Reference
description: This is the main page of Dynamsoft Code Parser SDK .NET Edition API Reference.
keywords: api reference, .NET
needAutoGenerateSidebar: true
needGenerateH3Content: true
---

# API Reference - Dynamsoft Code Parser SDK .NET Edition

## Classes

* [`CodeParserModule`]({{ site.dcp_dotnet_api }}code-parser-module.html)
* [`ParsedResult`]({{ site.dcp_dotnet_api }}parsed-result.html)
* [`ParsedResultItem`]({{ site.dcp_dotnet_api }}parsed-result-item.html)

## Enumerations  

* [`EnumErrorCode`]({{ site.dcv_enumerations }}core/error-code.html?lang=dotnet)
* [`EnumMappingStatus`]({{ site.dcv_enumerations }}code-parser/mapping-status.html?lang=dotnet)
* [`EnumValidationStatus`]({{ site.dcv_enumerations }}code-parser/validation-status.html?lang=dotnet)
